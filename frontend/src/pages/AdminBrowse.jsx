...
export default function Admin() {
  ...
  const fetchData = async () => {
    try {
      const [statsRes, postsRes] = await Promise.all([
        axios.get('/api/posts/stats/admin'),
        axios.get('/api/posts/admin/all')
      ]);
  ...
  const handleFlag = async (id) => {
    try {
      await axios.patch(`/api/posts/admin/${id}`, { status: 'flagged' });
  ...
  const handleSaveNote = async (id) => {
    try {
      await axios.patch(`/api/posts/admin/${id}`, { adminNote });
  ...
  const handleExportCSV = async () => {
    try {
      const response = await axios.get('/api/posts/export/csv', {
        responseType: 'blob'
      });
